...
async function main() {
    program
        ...
        .option("-H, --usb-host-ip <host:ip>", "use host:ip device, defaut is 127.0.0.1:27042")
        .option("-f, --spawn", "use spawn restart app")
        .option("-o, --output-dex-path <path>", "output dir of the dex file to save, default is ./")
        .option("-d, --deep-dump", "use deep search to dump dex", false);
    program.parse();
    const opts = program.opts();
    let outputdexpath = opts.outputdexpath || process.cwd();
    let packageNameOrPid = program.args[0];
