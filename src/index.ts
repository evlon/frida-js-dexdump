...
export async function main() {
  program
    ...
    .option(
      ...
      "use host:ip device, frida server defaut is 127.0.0.1:27042, use adb forward tcp:27042 tcp:27042 first"
    )
    ...
    .option(
      "--include-system",
      "when --spawn enable, use this to get system apps"
    );

  program.parse();
  const opts = program.opts();

  let packageName: any = program.args[0];
  ...
  try {
    ...
    try {
      ...
      if (opts.usbDevice) {
        ...
        if (deviceList.length > 1) {
          ...
          targetDevice = deviceList.find(
            (d) => d.name == choiceDeviceName.name
          )!;
        } else {
          //只有一个，就它吧
    ...
    if (opts.spawn) {
      if (!packageName) {
        ...
        if (!opts.includeSystem) {
          appsInstaled = appsInstaled.filter(
            (d) =>
              d.parameters.sources &&
      ...
      if(opts.frontApp){
        ...
        let foundPkgList = appProcessList.filter(
          (d) =>
            (d.parameters.applications ? d.parameters.applications[0] : "") ===
            packageName
        );
      ...
      if (targetPID === 0) {
        ...
        let cpn = await inquirer.prompt({
          ...
          choices: appProcessList.map(
            (d) =>
              `${d.pid}:${
                d.parameters.applications ? d.parameters.applications[0] : "nop"
              }-${d.name}`
          ),
        });
    ...
    try {
      ...
      script = await session.createScript(
        fs.readFileSync(agentJsFilePath, { encoding: "utf8" })
      );
