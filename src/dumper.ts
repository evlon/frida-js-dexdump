...
export interface AgentRpc {
  searchdex(deepSearch: boolean): Promise<any[]>;
  memorydump(
    address: number | NativePointerValue,
    size: number
  ): Promise<ArrayBuffer>;
  stopthreads(): Promise<void>;
}
...
export class Dumper {
  ...
  async dump(outputDir: string, deepSearch: boolean = false) {
    ...
    console.log(
      `[*] Successful found ${ranges.length} dex, used ${
